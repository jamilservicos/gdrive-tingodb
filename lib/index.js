 ...
 * @param params.deprecatedCollections - collections for block sync
 * @constructor
 */
var GdriveTingodb = function(params) {
	this.dbPath = params.dbPath;
	this.dbGoogle = params.dbGoogle;
	this.deprecatedCollections = params.deprecatedCollections || {};
...
GdriveTingodb.prototype.sync = function (params) {
    if (this.queue.workersList().length || this.deprecatedCollections[params.colName])
        return;
...
GdriveTingodb.prototype.upDownCols = function() {
	...
	.then(result => {
		...
		return this.getDriveFileLists({
			fn: item => {
				...
				if (dir.has(item.title) || this.deprecatedCollections[item.title])
					return Promise.resolve();
	...
	.then(() => {
		return Promise.all(_.map(dbItems, i => {
			if (downLoadedCollections[i] || this.deprecatedCollections[i])
				return Promise.resolve();

			var dest = `${this.dbPath}/${i}`;
